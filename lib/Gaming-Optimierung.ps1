...
$Config = @{
    ...
    # 0x26 = Short Fixed, 2:1 Ratio: stabiles Gaming-Optimum (Stand 2026)
    # 0x2A (3:1) verursacht Mikrostutter + Audio-Aussetzer bei Overlays - daher 0x26
...
function Enable-HAGS {
    ...
    try {
        ...
        $gpuLoad = $null
        try {
            # GPU-Auslastung: zuerst WMI (locale-unabhaengig), dann PerfCounter als Fallback
            $gpuWMI = Get-CimInstance -Namespace "root\cimv2" -ClassName Win32_PerfFormattedData_GPUPerformanceCounters_GPUEngine -ErrorAction SilentlyContinue |
                      Where-Object { $_.Name -match "engtype_3D" } |
                      Measure-Object -Property UtilizationPercentage -Average
            if ($gpuWMI -and $gpuWMI.Average -gt 0) {
                $gpuLoad = [math]::Round($gpuWMI.Average, 1)
            } else {
                # Fallback: englischer PerfCounter-Pfad (funktioniert auf EN-Windows)
                $gpuCounter = Get-Counter "\GPU Engine(*engtype_3D)\Utilization Percentage" -SampleInterval 1 -MaxSamples 1 -ErrorAction SilentlyContinue
                if ($gpuCounter) {
                    $gpuLoad = [math]::Round(($gpuCounter.CounterSamples | Measure-Object -Property CookedValue -Average).Average, 1)
                }
            }
        } catch {
            # Kein Fehler ausgeben - GPU-Load ist optional (Fallback: $null = kein Check)
        }
...
function Set-NetworkOptimizations {
    ...
    if ($Config.ReceiveWindowTuning) {
        try {
            ...
            # ECN (Explicit Congestion Notification): DEAKTIVIERT fuer Gaming
            # Obwohl ECN theoretisch Bufferbloat verhindert, verursacht es in der Praxis
            # hoehere Pings bei Gaming weil:
            # - Manche ISP-Middleboxes und CDN-Firewalls (Aktivision, AWS, Azure)
            #   haben fehlerhafte ECN-Implementierungen -> extra Retransmit-Versuche
            # - CoD-Nutzer berichten 10-20ms hoehere Pings mit ECN=enabled
            # - RFC 3168 Compliance ist auf Game-Server-Infrastruktur nicht garantiert
            # Fuer Bufferbloat-Schutz ohne ECN: autotuninglevel=normal + CUBIC genuegen.
            & netsh int tcp set global ecncapability=disabled  2>&1 | Out-Null
            Write-OK "ECN deaktiviert (Gaming-Empfehlung: verhindert ECN-Middlebox-Pingprobleme)."
            Write-Info "Hinweis: ECN=enabled kann auf manchen ISP-Strecken 10-20ms extra Ping verursachen."
    ...
    # DNS-Benchmark: schnellsten Server testen und automatisch setzen
    if ($Config.DNSOptimieren) {
        try {
            Write-Info "DNS-Benchmark laeuft - teste 8 Server (ca. 15 Sekunden)..."

            # Kandidaten: alle gaengigen Gaming-optimierten DNS-Server
            $dnsKandidaten = @(
                @{ Name = "Cloudflare";      Primary = "1.1.1.1";    Secondary = "1.0.0.1" },
                @{ Name = "Cloudflare-Alt";  Primary = "1.0.0.1";    Secondary = "1.1.1.1" },
                @{ Name = "Google";          Primary = "8.8.8.8";    Secondary = "8.8.4.4" },
                @{ Name = "Google-Alt";      Primary = "8.8.4.4";    Secondary = "8.8.8.8" },
                @{ Name = "Quad9";           Primary = "9.9.9.9";    Secondary = "149.112.112.112" },
                @{ Name = "OpenDNS";         Primary = "208.67.222.222"; Secondary = "208.67.220.220" },
                @{ Name = "AdGuard";         Primary = "94.140.14.14";   Secondary = "94.140.15.15" },
                @{ Name = "ISP-Standard";    Primary = $null;            Secondary = $null }
            )

            # Testdomains: alle relevanten Gaming-Plattformen
            # Wichtig: DNS-Latenz zu diesen Domains bestimmt wie schnell
            # Matchmaking, Login und erste Server-Verbindung aufgebaut werden.
            # Jede Plattform hat eigene CDN-Infrastruktur -> unterschiedliche
            # DNS-Antwortzeiten je nach Anbieter und Standort.
            $testDomains = @(
                # Activision / Battle.net (CoD direkt + Blizzard Launcher)
                "activision.com",
                "blizzard.com",
                # Steam (CoD via Steam)
                "store.steampowered.com",
                "steamcommunity.com",
                # Xbox / Game Pass (CoD via Xbox App)
                "xbox.com",
                "xboxlive.com",
            ...
            foreach ($dns in $dnsKandidaten) {
                if ($null -eq $dns.Primary) {
                    # ISP-Standard: aktuellen DNS auslesen
                    $currentDNS = (Get-DnsClientServerAddress -AddressFamily IPv4 |
                        Where-Object { $_.ServerAddresses.Count -gt 0 } |
                        Select-Object -First 1).ServerAddresses
                    if (-not $currentDNS) { continue }
                    $testIP = $currentDNS[0]
                } else {
                    $testIP = $dns.Primary
                }
                ...
                foreach ($domain in $testDomains) {
                    try {
                        ...
                        # DNS-Lookup erzwingen (Cache umgehen durch Prefix)
                        $result = Resolve-DnsName -Name $domain -Server $testIP -Type A `
                                  -DnsOnly -NoHostsFile -ErrorAction SilentlyContinue
                        $start.Stop()
                        if ($result) {
                            $gesamtLatenz += $start.ElapsedMilliseconds
                            $erfolge++
                        }
                    } catch { }
                }
                ...
                if ($erfolge -gt 0) {
                    $avgLatenz = [math]::Round($gesamtLatenz / $erfolge, 1)
                    ...
                    Write-Info "  $($dns.Name.PadRight(15)) $($testIP.PadRight(17)) Avg: ${avgLatenz}ms ($erfolge/$($testDomains.Count) OK)"
                }
            }

            if ($ergebnisse.Count -gt 0) {
                # Besten DNS waehlen: niedrigste Latenz mit allen Anfragen beantwortet
                $bester = $ergebnisse | Where-Object { $_.Erfolge -eq $testDomains.Count } |
                          Sort-Object AvgMs | Select-Object -First 1

                if (-not $bester) {
                    # Fallback: bester mit mindestens 3/4 Erfolgen
                    $bester = $ergebnisse | Where-Object { $_.Erfolge -ge 3 } |
                              Sort-Object AvgMs | Select-Object -First 1
                }

                if ($bester) {
                    Write-OK "Schnellster DNS: $($bester.Name) ($($bester.IP)) - $($bester.AvgMs)ms"

                    $adapters = Get-NetAdapter | Where-Object { $_.Status -eq "Up" }
                    $servers  = @($bester.IP)
                    if ($bester.IP2) { $servers += $bester.IP2 }

                    foreach ($adapter in $adapters) {
                        Set-DnsClientServerAddress -InterfaceIndex $adapter.InterfaceIndex `
                            -ServerAddresses $servers -ErrorAction SilentlyContinue
                        Write-OK "DNS gesetzt fuer: $($adapter.Name)"
                    }
                    & ipconfig /flushdns | Out-Null
                    Write-OK "DNS-Cache geleert."
                    Write-Info "Tipp: DNS-Einstellung ist standort- und ISP-abhaengig. Erneut testen nach Router-Wechsel."
                } else {
                    Write-Warn "Kein zuverlaessiger DNS gefunden - Einstellung unveraendert."
                }
            }
        }
        catch { Write-Warn "DNS-Benchmark fehlgeschlagen: $_" }
    } else { Write-Skip "DNS-Benchmark uebersprungen (DNSOptimieren = false)." }
...
$gameProcesses = @(
    ...
    # CoD via Xbox App / Game Pass
    "GamingServices","XboxPcApp","MicrosoftGamingOverlay","GameBar",
    "AppVShNotify"   # Xbox App Activity-Prozess
)
...
function Set-IRQPrioritization {
    ...
    try {
        ...
        $nicPath = "HKLM:\SYSTEM\CurrentControlSet\Control\Class\{4d36e972-e325-11ce-bfc1-08002be10318}"  # NIC-GUID (nicht GPU {4d36e968}!)
        ...
        try {
            ...
            try {
                Set-NetTCPSetting -SettingName "InternetCustom" `
                    ...
                    -ErrorAction SilentlyContinue
                # ECN=Disabled: konsistent mit netsh ecncapability=disabled (Gaming-Empfehlung)
                ...
                # AutomaticUseCustom=Enabled: Windows wendet InternetCustom-Profil auf
                # Verbindungen mit RTT > 10ms an. Ohne das wuerde InternetCustom ignoriert.
                Set-NetTCPSetting -SettingName "Automatic" `
                    -AutomaticUseCustom Enabled `
                    -ErrorAction SilentlyContinue
                Write-OK "Set-NetTCPSetting: InternetCustom aktiviert, ScalingHeuristics=Disabled."
                Write-Info "AutomaticUseCustom=Enabled: InternetCustom gilt fuer alle Verbindungen > 10ms RTT."
...
function Set-DefenderGameExclusions {
    ...
    try {
        # ── Statische Basispfade (alle Launcher) ─────────────────────────────────
        $gamePaths = [System.Collections.Generic.List[string]]@(
            ...
            "C:\Games",
            "C:\XboxGames"   # Xbox App / Game Pass Standard-Installpfad
        )

        # ── Dynamische Suche: CoD ueber Steam ─────────────────────────────────────
        # Steam kann auf beliebiger Festplatte installiert sein -> Registry auslesen
        $steamPaths = @()
        try {
            # Steam-Bibliothekspfade aus libraryfolders.vdf auslesen
            $steamReg = Get-ItemProperty "HKLM:\SOFTWARE\WOW6432Node\Valve\Steam" -ErrorAction SilentlyContinue
            if (-not $steamReg) { $steamReg = Get-ItemProperty "HKLM:\SOFTWARE\Valve\Steam" -ErrorAction SilentlyContinue }
            if ($steamReg.InstallPath) {
                $libFile = Join-Path $steamReg.InstallPath "steamapps\libraryfolders.vdf"
                if (Test-Path $libFile) {
                    $libContent = Get-Content $libFile -Raw -ErrorAction SilentlyContinue
                    # Alle "path" Eintraege extrahieren
                    $matches2 = [regex]::Matches($libContent, '"path"\s+"([^"]+)"')
                    foreach ($m in $matches2) {
                        $libPath = $m.Groups[1].Value -replace "\\","\"
                        $steamApps = Join-Path $libPath "steamapps\common"
                        if (Test-Path $steamApps) { $steamPaths += $steamApps }
                    }
                }
            }
        } catch {}
        foreach ($sp in $steamPaths) {
            if ($sp -notin $gamePaths) { $gamePaths.Add($sp) }
        }

        # ── Dynamische Suche: CoD ueber Battle.net auf custom Pfad ────────────────
        try {
            $bnetReg = Get-ItemProperty "HKCU:\SOFTWARE\Blizzard Entertainment\Battle.net\Appearance" -ErrorAction SilentlyContinue
            if ($bnetReg.InstallPath -and (Test-Path $bnetReg.InstallPath)) {
                if ($bnetReg.InstallPath -notin $gamePaths) { $gamePaths.Add($bnetReg.InstallPath) }
            }
            # CoD-spezifischen Installpfad suchen
            $codRegPaths = @(
                "HKLM:\SOFTWARE\Activision\Call of Duty",
                "HKLM:\SOFTWARE\WOW6432Node\Activision\Call of Duty",
                "HKCU:\SOFTWARE\Activision\Call of Duty"
            )
            foreach ($rp in $codRegPaths) {
                $codReg = Get-ItemProperty $rp -ErrorAction SilentlyContinue
                if ($codReg.InstallPath -and (Test-Path $codReg.InstallPath)) {
                    if ($codReg.InstallPath -notin $gamePaths) { $gamePaths.Add($codReg.InstallPath) }
                }
            }
        } catch {}

        # ── Dynamische Suche: CoD ueber Xbox App / Game Pass ──────────────────────
        try {
            # Xbox/GamePass Pakete via Get-AppxPackage finden
            $xboxCod = Get-AppxPackage -Name "*CallofDuty*" -ErrorAction SilentlyContinue
            if (-not $xboxCod) { $xboxCod = Get-AppxPackage -Name "*Activision*" -ErrorAction SilentlyContinue }
            foreach ($pkg in $xboxCod) {
                if ($pkg.InstallLocation -and (Test-Path $pkg.InstallLocation)) {
                    $gamePaths.Add($pkg.InstallLocation)
                    Write-OK "Xbox/GamePass CoD gefunden: $($pkg.InstallLocation)"
                }
            }
            # WindowsApps Ordner pauschal (Microsoft Store Spiele)
            $winAppsPath = "$env:ProgramFiles\WindowsApps"
            if (Test-Path $winAppsPath) { $gamePaths.Add($winAppsPath) }
        } catch {}

        # ── Ausnahmen setzen (nur existierende Pfade) ──────────────────────────────
        $added = 0
        foreach ($path in $gamePaths) {
            ...
            if (Test-Path $path) {
                ...
                Write-Skip "Nicht gefunden: $path"
            }
        }

        # ── Prozess-Ausnahmen: alle CoD-Plattformen ───────────────────────────────
        $gameProcesses = @(
            # CoD via Battle.net / direkt
            "cod.exe","cod_launcher.exe","cod_client.exe",
            "HQ.exe","HQGame.exe",                          # CoD HQ = MW2/MW3/Warzone 2024+
            "BlackOpsColdWar.exe","ModernWarfare.exe",
            "ModernWarfare2.exe","ModernWarfare3.exe",
            "Warzone.exe","WarzoneCaldera.exe",
            # CoD via Steam (Steam-Wrapper)
            "steam.exe","steamwebhelper.exe","GameOverlayRenderer.exe",
            # CoD via Xbox App / Game Pass
            "GamingServices.exe","XboxPcApp.exe","MicrosoftGamingOverlay.exe",
            "GameBar.exe",
            # Launcher allgemein
...
function Disable-PowerThrottling {
    ...
    try {
        ...
        # EcoQoS systemweit deaktivieren (Registry-Methode, Windows 11 24H2+)
        # EcoQoS ist ein Effizienz-Modus der Prozesse auf E-Kerne verlagert
        # Fuer Gaming: deaktivieren damit CoD nicht auf E-Kerne gezwungen wird
        ...
        Set-ItemProperty -Path $powerThrottlePath -Name "PowerThrottlingOff" -Value 1 -Type DWord -ErrorAction SilentlyContinue
        Write-OK "EcoQoS / PowerThrottling systemweit deaktiviert (PowerThrottlingOff=1)."
        Write-Info "CoD und andere Spielprozesse werden nicht auf Effizienz-Kerne (E-Cores) verlagert."
...
function Set-Win32PrioritySeparation {
    ...
    try {
        ...
        # 0x26 (Hex) = 38 (Dezimal) = Short, Fixed, Medium Foreground Boost
        # Bedeutung der Bits:
        ...
        #   Bit 1-0 (10) = Medium Foreground Boost: Spiel bekommt 2x mehr CPU-Zeit als Hintergrund
        ...
        Write-Info "Aktueller Wert: $currentVal -> Neuer Wert: 0x26 (38 dezimal)"

        Set-ItemProperty -Path $prioPath -Name "Win32PrioritySeparation" -Value 0x26 -Type DWord

        Write-OK "Win32PrioritySeparation auf 0x26 gesetzt (Short, Fixed, Medium Foreground Boost)."
        Write-OK "CoD erhaelt 2x mehr CPU-Zeit gegenueber Hintergrundprozessen."
...
function Invoke-UndoOptimizations {
    ...
    try {
        # GlobalUserDisabled wird nicht mehr gesetzt (seit v20) - nur sicherheitshalber entfernen
        ...
        Write-OK "Gaming Quiet Mode-Einstellungen zurueckgesetzt."
    } catch {}


    # VBS/HVCI reaktivieren (nur wenn Pfad existiert = VBS war vorher aktiv)
    try {
        $hvciPath = 'HKLM:\SYSTEM\CurrentControlSet\Control\DeviceGuard\Scenarios\HypervisorEnforcedCodeIntegrity'
        $vbsPath  = 'HKLM:\SYSTEM\CurrentControlSet\Control\DeviceGuard'
        # Pfade anlegen falls nicht vorhanden (neuer PC ohne VBS-Historie)
        ...
        Set-ItemProperty -Path $vbsPath  -Name 'EnableVirtualizationBasedSecurity' -Value 1 -Type DWord -ErrorAction SilentlyContinue
...
function Set-GamingQuietMode {
    ...
    try {
        ...
        # Xbox Mode (NEU Mai 2026, Windows 11 24H2/25H2 Build 26200.8328+)
        # Reduziert Hintergrund-RAM um bis zu 9,3% und steigert FPS bis 8,6%
        # Wird per graduellem Rollout verteilt - Script erkennt Verfuegbarkeit
        $winBuild = [System.Environment]::OSVersion.Version.Build
        $hasFSE = (Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion" -ErrorAction SilentlyContinue).UBR
        $fullBuild = "$winBuild.$hasFSE"

        if ($winBuild -ge 26100) {
            Write-Info "Xbox Mode (Mai 2026): Pruefe Verfuegbarkeit..."
            $xboxModeKey = "HKCU:\Software\Microsoft\Windows\CurrentVersion\GameBar"
            if (-not (Test-Path $xboxModeKey)) { New-Item -Path $xboxModeKey -Force | Out-Null }
            # XboxMode enablen falls Schluessel durch Windows Update freigeschaltet wurde
            Set-ItemProperty -Path $xboxModeKey -Name "AutoGameModeEnabled" -Value 1 -Type DWord -ErrorAction SilentlyContinue
            Write-OK "Xbox Mode-Schluessel vorbereitet (aktiviert sobald Windows-Update KB5083631+ installiert ist)."
            Write-Info "Xbox Mode manuell aktivieren: Einstellungen > Gaming > Xbox Mode (nach Mai 2026 Update)."
            Write-Info "Vorteil: bis zu 9,3% weniger RAM-Verbrauch und 8,6% mehr FPS im Vollbild."
        } else {
            Write-Skip "Xbox Mode: Windows 11 24H2+ erforderlich (aktueller Build: $winBuild)."
        }
        ...
        # Statische Launcher + dynamisch gefundene Xbox-App-Exe
        $launchers = [System.Collections.Generic.List[string]]@(
        ...
        # Xbox App dynamisch suchen
        try {
            $xboxApp = Get-AppxPackage -Name "Microsoft.GamingApp" -ErrorAction SilentlyContinue
            if ($xboxApp) {
                $xboxExe = Join-Path $xboxApp.InstallLocation "Gaming.Desktop.X64.Launcher.exe"
                if (Test-Path $xboxExe) { $launchers.Add($xboxExe) }
            }
        } catch {}
        ...
        # Hintergrund-Apps: NICHT global deaktivieren (würde Xbox Mode Mai 2026 blockieren)
        # GlobalUserDisabled=1 verhindert dass Xbox Mode / Gaming Services im Hintergrund laufen
        # Stattdessen: AutoGameModeEnabled=1 (oben bereits gesetzt) uebernimmt diese Aufgabe
        # Windows priorisiert Spiele automatisch wenn AutoGameModeEnabled aktiv ist
        Write-Info "Hintergrund-Apps: via AutoGameModeEnabled=1 priorisiert (kompatibel mit Xbox Mode)."
...
function Disable-FullscreenOptimizations {
    ...
    try {
        ...
        # Launcher-EXEs dynamisch suchen (Steam, BNet, Xbox, Epic)
        $gameExes = [System.Collections.Generic.List[string]]@(
            "$env:ProgramFiles\Battle.net\Battle.net Launcher.exe",
            "$env:ProgramFiles (x86)\Battle.net\Battle.net Launcher.exe",
            "$env:ProgramFiles\Steam\steam.exe",
            "$env:ProgramFiles (x86)\Steam\steam.exe",
        ...
        # Xbox App / Game Pass Launcher
        $xboxExePaths = @(
            "$env:ProgramFiles\WindowsApps",
            "$env:LOCALAPPDATA\Microsoft\WindowsApps"
        )
        foreach ($xp in $xboxExePaths) {
            if (Test-Path $xp) {
                Get-ChildItem -Path $xp -Filter "*.exe" -Recurse -ErrorAction SilentlyContinue |
                    Where-Object { $_.Name -match "XboxPcApp|GamingServices|Xbox" } |
                    Select-Object -First 3 | ForEach-Object { $gameExes.Add($_.FullName) }
            }
        }
